...
# Output paths.
OUTPUT_DIR = "outputs_merged_classes"
...
# Label mappings of the 4 merged parent classes.
LABEL_MAP = {
...
def preprocess_data(data_path, max_per_class):
    ...
    # Merge attack subtypes into their parent categories using LABEL_MAP.
    # The merge is applied before the global per-class cap so that the cap
    # operates on the parent class total, not on each subtype individually.
    classes_before = data['Label'].nunique()
    data['Label'] = data['Label'].replace(LABEL_MAP)
    classes_after = data['Label'].nunique()
    print(f"Merged subtypes: {classes_before} classes -> {classes_after} classes")
